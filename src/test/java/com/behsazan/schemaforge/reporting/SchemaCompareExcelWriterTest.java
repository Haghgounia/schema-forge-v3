...
class SchemaCompareExcelWriterTest {
    ...
    private final SchemaCompareExcelWriter writer = new SchemaCompareExcelWriter();

    @Test
    void createsComparisonWorkbookAndMarksDifferences() throws Exception {
        TableDefinition document = new TableDefinition(
                "CIF",
                "CUSTOMERS",
                "Customers",
        ...
        Column databaseColumn = new Column(
                Identifier.of("CUSTOMER_ID"),
                DataType.numeric("NUMBER", 10, null),
                false,
                new DefaultValue(null),
                new Description("Customer identifier"),
                false,
                1);

        Table database = Table.builder("CIF", "CUSTOMERS")
                .addColumn(databaseColumn)
                .build();

        byte[] bytes = writer.write(document, database, Map.of("CUSTOMER_ID", 74));

        assertThat(bytes).isNotEmpty();
        try (XSSFWorkbook workbook = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
            assertThat(workbook.getSheet("CUSTOMERS").getRow(1).getCell(0).getNumericCellValue()).isEqualTo(74);
            assertThat(workbook.getSheet("CUSTOMERS").getRow(1).getCell(19).getStringCellValue())
                    .contains("DATA_TYPE")
